...
class Helper {
    ...
    public static function promocodeStatuslabel($activeStatus) {
        if ($activeStatus == 'active') {
            return '<span class="label label-success">' . Config::get('constants.PROMO_STATUSES')[$activeStatus] . '</span>';
        } else {
            return '<span class="label label-warning">' . Config::get('constants.PROMO_STATUSES')[$activeStatus] . '</span>';
        }
    }
    ...
    public static function activeConnectedPeriodlabel($period) {
        // return Config::get('constants.CONNECTED_PERIODS')[6];
