...
class FamilyTreeController extends BaseController
{
            ...
            if(!empty($currentUser->partnerInfo)){
                ...
                if(!is_null($currentUser->partnerInfo->fid) || !is_null($currentUser->partnerInfo->mid)){
                    if(!is_null($currentUser->partnerInfo->fid) && !is_null($currentUser->partnerInfo->mid)){
                        ...
                        //foreach ($currentUser->partnerInfo->fatherInfo->childInfosForFather()->get() as $key => $siblingInfo) {
                        ...
                        if(!is_null($currentUser->partnerInfo->mid)){
                            ...
                            foreach($childrens as $key=>$siblingInfo){
                        //foreach ($currentUser->partnerInfo->motherInfo->childInfosForMother()->get() as $key => $siblingInfo) {
                            //sibling in law infos
                            $data=$this->processData($siblingInfo,16);
                            array_push($datas,$data);
                        }
                ...
                foreach($childrens as $key=>$childInfo){                
                    if($childInfo->gender=='male'){
                        ...
                        if($childInfo->pid != null){
                            ...
                            $data=$this->processData($sonspartner,$relationID);
                            array_push($datas,$data); 
                            ...
                            foreach($childInfo->childInfosForFather()->get() as $gkey=>$grandChildInfo){
                                if($grandChildInfo->gender=='male'){                         
                                    ...
                                    if($grandChildInfo->pid != null){
                                        ...
                                        $data=$this->processData($grandSonsPartner,$relationID);
                                        array_push($datas,$data); 
                                    }
                                ...
                                if($grandChildInfo->gender=='female'){                         
                                    ...
                                    if($grandChildInfo->pid != null){
                                        ...
                                        $relationID = 0;
                                        if ($grandDaughtersPartner->relation_id == 14) { $relationID = 35;  } //wife       
                                        if ($grandDaughtersPartner->relation_id == 26) { $relationID = 36;  } //Husband       
                                        if ($grandDaughtersPartner->relation_id == 27) { $relationID = 37;  } //Partner

                                        $data=$this->processData($grandDaughtersPartner,$relationID);
                                        array_push($datas,$data); 
                                    }
                                }
                            }
                    ...
                    if($childInfo->gender=='female'){
                        ...
                        if($childInfo->pid != null){
                            ...
                            $data=$this->processData($daughterspartner,$relationID);
                            array_push($datas,$data); 
                        }
    ...
    public function addAsNewFamilyTree($request,$newUser)
    {
        ...
        if(empty($isRelativeInfoExist)){
            ...
            if($relativeInfo->dob != null){
               $relativeInfo->dob=date_format(date_create( $request->get('relation_dob')),'Y-m-d');
            }
          
            // if ($request->relation_id == 14 || $request->relation_id == 26 || $request->relation_id == 27 ) {
    ...
    public function addAsNewUser($request,$relatedUser)
    {
        ...
        if (empty($isUserExist)) {
            ...
            if($request->relation_dob != null){
                 $userInfo->dob=date_format(date_create( $request->get('relation_dob')),'Y-m-d');
            }            
            $unique_password = substr(str_shuffle('ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz'), 0,4).substr(str_shuffle('!@#$%^&*()'), 0,1).substr(str_shuffle('ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz'), 0,4).rand(1,999);
    ...
    public function deleteMember($id){
        ...
        // }
        // return (['success'=>false,'msg'=>'Failed to delete data you can delete you patner, father and mother now.']);
    }
    ...
    public function get_video(Request $request){
        if($request->user_id != Auth::user()->id){
                $story = Story::where('user_id',$request->user_id)->first();
    
                if($story){
                    return response()->json(['msg'=>'Success','data'=>$story]);   
                }
                else{
                    return response()->json(['msg'=>'Error','data'=>'No Records Found']);
                }
